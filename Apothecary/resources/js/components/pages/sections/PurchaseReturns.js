...
export default class PurchaseReturns extends Component {
    ...
    constructor (props) {
      ...
      componentDidMount () {
        ...
        axios.get(`/api/getPurchaseReturns/${this.state.Pharm_Id}`).then(response => {
          this.setState({
    ...
    orderInterval(){
      ...
      axios.get(`/api/getOrderList/${this.state.pharm_id}/${this.state.status_id}`).then(response => {
        if(response.data.length>0){
          toaster.notify("There are returns pending that need to be reviewed, Please visit the returns tab ASAP.", {
                  ...
                  <div className='col-md-9'>
                    <div className='card'>
                       <div className='card-header' id='col' ><MDBIcon id='back' icon='arrow-left' size='2x' style={{ cursor: 'pointer' }} onClick={() => this.props.history.goBack()}/><h4 id='tp'>REPORTS</h4></div>
                          ...
                          <h4 id='tp'>Purchase Returns</h4><br></br>
                          <MDBTable striped responsive>
                            ...
                            <TableBody>
                             {this.state.purchaseReturns.map(returns => (
                                <tr key={returns.Id}>
                                <td>{returns.Purchase_Id}</td>
