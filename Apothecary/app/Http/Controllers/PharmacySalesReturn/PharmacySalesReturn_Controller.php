...
class PharmacySalesReturn_Controller extends Controller
{
	 ...
	 public function store(Request $request)
    {
          ...
          if($validator->fails())
        {
          ...
          if($Sale_details){
            ...
            if($request->stock_type==$Sale_details->stock_type){
             ...
             if($Sale_return->save()){
              ...
              $returnData = array(
                'ErrorMessage'=>'Entered Product Type does not match the product type of sold product ');
                return response()->json($returnData,202);              
            ...
            $returnData = array(
              'ErrorMessage'=>'Cannot find the respective data against the Entered Sale Id and Stock Id');
              return response()->json($returnData,202);  
    ...
    public function show($pharmid)
    {
         ...
         ->select('sale_return.Id','sale_return.Sale_Id','stock.Name as Product'
         ,'stock.Item_Description as Description'
         ,DB::raw("CASE WHEN sale_return.stock_type=0 THEN 'Unit'
         WHEN sale_return.stock_type=1 THEN 'Leaf' 
         WHEN sale_return.stock_type=2 THEN 'Pack' END as Product_Type")
         ...
         ->orderBy('sale_return.Return_Date','desc')
         ->get();
