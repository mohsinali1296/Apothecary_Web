...
public function store(Request $request)
    {
          ...
          if($validator->fails())
        {
            ...
            $Purchase_details= DB::table('purchase_details')->select('purchase_details.*')
            ->where('purchase_details.Purchase_Id','=',$request->Purchase_Id)
            ->where('purchase_details.Stock_Id','=',$request->Stock)
            ->first();

            if($Purchase_details){

                if($request->unit_Qty<=$Purchase_details->unit_Qty){
                ...
                $Purchase_return->TotalPrice = $request->unit_Qty* $Purchase_details->unit_BuyPrice;
                ...
                }else if($request->unit_Qty>$Purchase_details->unit_Qty){
    
                    $returnData = array(
                        'ErrorMessage'=>'Entered Quantity is greater than Purchased Quantity');
                ...
                $returnData = array(
                    'ErrorMessage'=>'Cannot find respective data against the entered Purchase Id and Stock Id');
                    return response()->json($returnData,202);
            }
    ...
    public function show($pharmid)
    {
       ...
       ->select('purchase_return.Id','purchase_return.Purchase_Id','stock.Name as Product'
       ...
       DB::raw("DATE_FORMAT(purchase_return.Return_Date, '%T') as Time"))
        ...
        ->orderBy('purchase_return.Return_Date','desc') 
         ->get();
    ...
    public function update(Request $request,$id)
    {
          ...
          if($validator->fails())
       
           {
            ...
            if(($Purchase_return->unit_Qty+$request->unit_Qty)<=$Purchase_details->unit_Qt){
                ...
                if($Purchase_return->unit_Qty < $request->unit_Qty){
                    ...
                    $Purchase_return->TotalPrice = $Purchase_return->unit_Qty * $Purchase_details->unit_BuyPrice;
                    $Purchase_return->Return_Date = $request->Return_Date;
                    $Purchase_return->Employee_Id = $request->Employee_Id;
                ...
                $returnData = array(
                    'ErrorMessage'=>'Entered Quanttity is greater than Purchased Quantity');
                    return response()->json($returnData,202);
